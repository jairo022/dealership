...
const Touring = () => {
  ...
  useEffect(() => {
    const fetchTouringMotorcycles = async () => {
      try {
        const touringRef = ref(database, 'motorcycles');
        const snapshot = await get(touringRef);

        if (snapshot.exists()) {
          const touringBikesData = await Promise.all(
            Object.keys(snapshot.val()).map(async (key) => {
              const bike = snapshot.val()[key];
              if (bike.vehicleType === 'touring') {
          ...
          setMotorcycles(touringBikesData.filter(bike => bike !== null));
        } else {
          console.log("No data available for Touring motorcycles.");
        }
      } catch (error) {
        console.error("Error fetching touring motorcycles:", error);
      }
    };

    fetchTouringMotorcycles();
  }, []);
