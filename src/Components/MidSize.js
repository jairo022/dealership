    ...
    const fetchTrucks = async () => {
      try {
        ...
        if (snapshot.exists()) {
          ...
          console.log("No midsize trucks available");
        }
      } catch (error) {
        console.error("Error fetching midsize trucks:", error);
      }
    };
    ...
    <div>
      ...
      <div className="car-container">
        {trucks.length > 0 ? (
          ...
          <div>No data available for midsize trucks.</div>
        )}
      </div>
    </div>
  );
